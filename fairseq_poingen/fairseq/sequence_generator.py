...
class SequenceGenerator(nn.Module):
    ...
    def _generate(
        ...
        encoder_outs_with_src_tokens = self.model.forward_encoder(
            src_tokens=encoder_input["src_tokens"],
            src_lengths=encoder_input["src_lengths"],
        )

        encoder_outs = [encoder_outs for encoder_outs, src_tokens in encoder_outs_with_src_tokens]

        # placeholder of indices for bsz * beam_size to hold tokens and accumulative scores
        new_order = torch.arange(bsz).view(-1, 1).repeat(1, beam_size).view(-1)
        new_order = new_order.to(src_tokens.device).long()
        encoder_outs = self.model.reorder_encoder_out(encoder_outs, new_order)
        encoder_outs_with_src_tokens = [(encoder_outs[i], encoder_outs_with_src_tokens[i][1])\
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            lprobs, avg_attn_scores = self.model.forward_decoder(
                tokens[:, : step + 1], encoder_outs_with_src_tokens, self.temperature
            )
...
class EnsembleModel(nn.Module):
    ...
    def forward_decoder(
        ...
        for i, model in enumerate(self.models):
            if self.has_encoder():
                encoder_out, src_tokens = encoder_outs[i]
            # decode each model
            if self.has_incremental_states():
